...
const setupWorkspace = (dbName) => {
  setWorkspace(dbName)
    ...
    .then(setProviders)
    .then(workspace => {
      workspace.connectors[0].provider.tweet()
      return workspace
    })
    .then(closeWorkspace)
}
...
const setAccounts = (workspace) => {
  ...
  return Promise.all(promises).then(() => workspace)
}
  ...
  workspace.connectors.forEach(connector => {
    ...
    workspace.docs
      ...
      .forEach(workflow => {
        ...
        connector.actions[workflow.action].workflows.push({doc:workflow})
      })
  })
...
process.on('beforeExit', () => accountsDB.close())

const closeWorkspace = (workspace) => workspace.db.close()

module.exports = {setupWorkspace, closeWorkspace, setWorkspace, setConnectors, setAccounts, setWorkflows}
