...
describe('social:workspaces', () => {
  ...
  beforeEach(() => {
    doc = {_id:'a'}
    docs = [
      {_id:'b', docType:'channel', dbName:'db1'},
      {_id:'c', docType:'workflow', connector:'d', channel:'b'},
  ...
  describe('add', () => {
    describe('new workspace', () => {
      test('should add workspace', () => {
        ...
        expect(workspaces.a.channels).toEqual({'b': {'_id': 'b', 'dbName': 'db1', 'docType': 'channel'}})
        expect(workspaces.a.connectors).toEqual({'d': {'_id': 'd', 'account': 'e', 'docType': 'connector'}})
        expect(workspaces.a.workflows).toEqual({'c': {'_id': 'c', 'channel': 'b', 'connector': 'd', 'docType': 'workflow'}})
        expect(workspaces.a.dbMap).toEqual({c:'db1'})
      })
    })
