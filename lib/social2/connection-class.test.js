...
describe('social:connection', () => {
  test('constructor', () => {
    ...
    expect(c._id).toEqual('c1')
    expect(c._deleted).toEqual(false)
  })

  describe('deleted connector', () => {
    test('should set _deleted', () => {
      const connector = newConnector()
      connector._deleted = true
      const c = new Connection(connector, newAccount())
      expect(c._deleted).toEqual(true)
    })
  })

  test('token & tokenSecret', () => {
    const c = new Connection(newConnector(), newAccount())
    expect(c.token).toEqual('t1')
    expect(c.tokenSecret).toEqual('ts1')
  })
  ...
  test('updateWorkflow new', () => {
    ...
    expect(c.workflows[0]).toEqual({_id:'wf1'})
  })
  ...
  test('updateWorkflow update', () => {
    ...
    expect(c.workflows.length).toEqual(1)
    expect(c.workflows[0]).toEqual({_id:'wf1', b:1})
  })
