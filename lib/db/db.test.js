  ...
  describe('workspaces', () => {
    ...
    describe('get', () => {
      ...
      test('should get', async () => {
        const next = jest.fn()
        const req = {inquiry: {id: seed.workspace.id}}
    ...
    describe('update', () => {
      ...
      beforeEach(async () => {
        ...
        next = jest.fn()
        req = {inquiry: {id: seed.workspace.id, table:'connectors'}}
      })

      test('should update value', async () => {
        req.data = {connectors:{twitterGruberjl:{platform:'new'}}}
      ...
      test('should add nested items', async () => {
        req.data = {connectors:{a:{id:'a', b:'new'}}}
        await workspaces.update(req, {}, next)
        expect(req.db).toBeDefined()
        expect(req.db.raw[0][0]).toEqual(null)
        expect(req.db.raw[0][1]).toEqual('OK')
      ...
      test('should remove a nested item', async () => {
        req.data = {connectors:{'22222222-2222-2222-2222-222222222222':null}}
        await workspaces.update(req, {}, next)
        expect(req.db).toBeDefined()
        expect(req.db.raw[0][0]).toEqual(null)
        expect(req.db.raw[0][1]).toEqual(6)
        expect(req.db.raw[1][0]).toEqual(null)
        expect(typeof req.db.raw[1][1]).toEqual('object')
        expect(next.mock.calls.length).toEqual(1)
      })
